...
const REGULATIONS = [
  ...
  {id:"pp28-2025",level:"Nasional",scope:"Perizinan berusaha",title:"PP 28 Tahun 2025",about:"Penyelenggaraan Perizinan Berusaha Berbasis Risiko melalui OSS; mencabut PP 5 Tahun 2021.",status:"Berlaku",verified:true,checked:"21 Agu 2026",location:"Indonesia",province:"",url:"https://peraturan.bpk.go.id/Details/319773/pp-no-28-tahun-2025"},
...
const DEFAULT_STATE = {
  ...
  taskFilter:"all",taskProgress:{},documentTask:null,documents:DEFAULT_DOCUMENTS.map(x=>({...x,taskIds:[...x.taskIds]})),docProject:"",docSearch:"",regFilter:"Semua",regSearch:"",regMode:"all",regProvince:"Semua daerah",docFilter:"Semua"
};
...
function loadState(){
  try{
    ...
    if(!["all","project"].includes(merged.regMode))merged.regMode="all";
    if(!["Semua","Nasional","Sektoral","Daerah"].includes(merged.regFilter))merged.regFilter="Semua";
    if(merged.regProvince!=="Semua daerah"&&!REGULATIONS.some(r=>r.province===merged.regProvince))merged.regProvince="Semua daerah";
    return merged;
  }
...
function renderSteps(){document.getElementById("steps").innerHTML=labels.map((x,i)=>`<button class="step ${i===state.step?"active":""} ${i<state.step?"done":""}" data-step="${i}" aria-label="Langkah ${i+1}: ${x}"><span>${i<state.step?"S":i+1}</span><small>${x}</small></button>`).join("");}
...
function renderLocation(){
  ...
  `<div class="map"><span class="island one"></span><span class="island two"></span><span class="island three"></span><span class="pin" aria-hidden="true"></span><div class="mapbox"><b>Polygon belum diunggah</b><small>GeoJSON atau KML · simulasi prototipe</small><button class="primary" data-action="upload-polygon">Unggah polygon</button></div></div>
...
function taskStatusActions(task,progress){
  if(progress.status==="not_started")return `<button data-action="start-task" data-id="${task.id}">Mulai tugas</button><button class="secondary declared-action" data-action="mark-existing" data-id="${task.id}">Saya sudah melakukan ini</button>`;
  if(progress.status==="self_declared")return `<button disabled>Sudah dilakukan</button><button class="secondary" data-action="undo-existing" data-id="${task.id}">Batalkan tanda</button>`;
  if(progress.status==="completed")return `<button disabled>Selesai terverifikasi</button><button class="secondary" data-action="reopen-task" data-id="${task.id}">Buka kembali</button>`;
  if(progress.status==="evidence_needed")return `<button disabled>Langkah selesai</button><button class="secondary" data-action="reopen-task" data-id="${task.id}">Periksa kembali</button>`;
  return `<button disabled>Sedang dikerjakan</button>`;
}
...
function renderRegulations(){
  ...
  const scopeCopy=state.regMode==="project"
    ? `Daftar ini dipersempit dari KBLI, sumber dampak, dan lokasi proyek ${esc(state.projectName)}. Kembali ke semua regulasi untuk menjelajahi katalog umum.`
    : "Katalog umum tidak mengikuti lokasi penapisan. Aturan daerah dapat dicari secara mandiri; gunakan mode proyek hanya ketika ingin melihat aturan yang terpicu oleh proyek aktif.";
  ...
  return `<div class="view-shell regulation-shell">${viewHeader("Basis regulasi","Katalog umum regulasi lingkungan dan perizinan berusaha, terpisah dari penapisan proyek.",officialLink(LINKS.jdihn,"Cari langsung di JDIHN","ghost"))}
    <section class="reg-scope" aria-label="Cakupan basis regulasi">
      <div class="segmented"><button class="${state.regMode==="all"?"active":""}" data-reg-mode="all">Semua regulasi</button><button class="${state.regMode==="project"?"active":""}" data-reg-mode="project">Relevan untuk proyek aktif</button></div>
      <p>${scopeCopy}</p>
    </section>
    <section class="coverage-note"><div><b>Cakupan indeks daerah saat ini</b><p>Aturan daerah yang sudah diperiksa baru mencakup Provinsi Kalimantan Timur, Kota Balikpapan, dan Kabupaten Kutai Kartanegara. Daerah lain tetap dapat ditelusuri melalui JDIHN atau JDIH pemerintah daerah; tidak dianggap otomatis tidak memiliki aturan.</p></div><span>Diperiksa 21 Agu 2026</span></section>
    <div class="view-card regulation-list"><div class="reg-toolbar"><label class="reg-search"><span class="visually-hidden">Cari regulasi</span><input id="reg-search" class="searchbox" value="${esc(state.regSearch)}" placeholder="Cari nomor, topik, sektor, atau daerah"></label>${state.regMode==="all"?`<label class="reg-location"><span>Wilayah indeks</span><select data-field="regProvince"><option>Semua daerah</option>${localProvinces.map(x=>`<option ${state.regProvince===x?"selected":""}>${esc(x)}</option>`).join("")}</select></label>`:""}</div>
      <div class="filter-chips">${filters.map(x=>`<button class="filter-chip ${state.regFilter===x?"active":""}" data-reg-filter="${x}">${x}</button>`).join("")}</div>
      <div class="reg-results"><div class="reg-count"><b>${regs.length} regulasi</b><span>${state.regMode==="project"?"berdasarkan proyek aktif":"dalam katalog terverifikasi"}</span></div>${regs.length?regs.map(r=>`<article class="reg-card"><span class="reg-level">${esc(r.level)}</span><div class="reg-card-copy"><h3>${esc(r.title)}</h3><p>${esc(r.about)}</p><small>${esc(r.scope)} · ${esc(r.location)} · Dicek ${esc(r.checked||"21 Agu 2026")}</small></div><div class="reg-card-side"><span class="status ${r.status==="Berlaku"||r.status.includes("perubahan")?"verified":"reviewing"}">${esc(r.status)}</span>${officialLink(r.url,"Buka sumber")}</div></article>`).join(""):`<div class="empty-note"><b>Tidak ada regulasi yang cocok</b><p>Ubah filter atau cari langsung pada basis resmi nasional dan JDIH pemerintah daerah.</p>${officialLink(LINKS.jdihn,"Cari di JDIHN")}</div>`}</div>
    </div>
  </div>`;
}
...
document.addEventListener("change",e=>{
  ...
  if(e.target.id==="doc-file-input"){
    ...
    if(task){const progress=progressForTask(task.id),total=guideForTask(task).steps.length;if(progress.done.length===total)state.taskProgress[taskProgressKey(task.id)]={status:"completed",done:progress.done,markedAt:progress.markedAt};}
...
document.addEventListener("click",e=>{
  const viewEl=e.target.closest("[data-view]");if(viewEl){e.preventDefault();state.view=viewEl.dataset.view;if(state.view==="documents")state.documentTask=null;if(viewEl.dataset.regOpen)state.regMode=viewEl.dataset.regOpen;if(viewEl.dataset.step!==undefined)state.step=+viewEl.dataset.step;renderView();return;}
