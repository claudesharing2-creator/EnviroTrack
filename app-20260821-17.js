...
const DEFAULT_STATE = {
  ...
  taskFilter:"all",taskProgress:{},documentTask:null,documents:DEFAULT_DOCUMENTS.map(x=>({...x,taskIds:[...x.taskIds]})),docProject:"",docSearch:"",regFilter:"Semua",regSearch:"",regMode:"all",regProvince:"Semua daerah",docFilter:"Semua",docStorage:{mode:"browser",rootName:"",rootFolder:"EnviroTrack",connected:false}
};
...
function renderCalendar(){
  ...
  return `<div class="view-shell">${viewHeader("Kalender kepatuhan","Tenggat tugas, pengujian, masa berlaku, dan laporan berkala dalam satu agenda.",'<button type="button" class="primary" data-action="add-event">+ Tambah agenda</button>')}<div class="calendar-layout"><div class="calendar"><div class="calendar-head">${["Sen","Sel","Rab","Kam","Jum","Sab","Min"].map(x=>`<span>${x}</span>`).join("")}</div><div class="calendar-grid">${cells.join("")}</div></div><aside class="view-card"><div class="label" style="margin-top:0"><b>Agenda mendatang</b><small>Agustus 2026</small></div><div class="agenda">${[["03 Agu","Laporan pemantauan SIMPEL","Compliance"],["07 Agu","Pengujian emisi genset","Operations"],["12 Agu","Review kajian air limbah","Environment"],["18 Agu","Rekonsiliasi manifest LB3","HSE"],["24 Agu","Laporan RKL-RPL semester","Environment"]].map((x,i)=>`<div class="agenda-item"><span class="dot ${i%2?"orange-dot":"green-dot"}"></span><div><b>${x[1]}</b><small>${x[0]} · ${x[2]}</small></div></div>`).join("")}</div></aside></div></div>`;
}
