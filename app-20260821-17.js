...
const ONBOARDING_KEY = "envirotrack-onboarding-v1";
const ONBOARDING_STEPS = [
  {id:"new-screening",title:"Mulai dari Penapisan baru",copy:"Masukkan KBLI, kapasitas, proses, dan lokasi. Dari sini EnviroTrack menyusun kesimpulan acuan serta checklist kerja.",tip:"Mulai dari data kegiatan; jangan langsung menebak dokumen.",selector:'[data-tour-id="new-screening"]'},
  {id:"studio",title:"Studio AMDAL adalah mode latihan",copy:"Gunakan lima fase ini untuk memahami tim, dokumen, pemeriksaan, dan keputusan tanpa mengirim data ke AMDALNet.",tip:"Cocok untuk belajar alur dan kesiapan internal.",selector:'[data-tour-id="studio"]'},
  {id:"documents",title:"Dokumen tetap terorganisasi lokal",copy:"Simpan bukti per proyek, ekspor state atau ZIP, dan pilih folder perangkat bila ingin membuat salinan terstruktur.",tip:"Tombol Hapus data aplikasi hanya menghapus data browser.",selector:'[data-tour-id="documents"]'},
  {id:"calendar",title:"Kalender mengikat tugas ke waktu",copy:"Tambahkan agenda, penanggung jawab, basis, dan bukti agar pekerjaan lingkungan tidak berhenti di checklist.",tip:"Kalender tersimpan di workspace lokal.",selector:'[data-tour-id="calendar"]'},
  {id:"regulations",title:"Basis regulasi untuk membaca sumber",copy:"Buka sumber aturan dan konteks daerah dari sini. Semua rekomendasi tetap perlu dicocokkan dengan kanal resmi.",tip:"EnviroTrack adalah alat bantu acuan, bukan penerbit keputusan.",selector:'[data-tour-id="regulations"]'}
];
let onboardingIndex=0;
let onboardingCloseTimer=null;
function onboardingSeen(){try{return localStorage.getItem(ONBOARDING_KEY)==="seen";}catch{return false;}}
function onboardingMarkSeen(){try{localStorage.setItem(ONBOARDING_KEY,"seen");}catch{}}
function onboardingTarget(){const item=ONBOARDING_STEPS[onboardingIndex];return item?document.querySelector(item.selector):null;}
function positionOnboardingSpotlight(){const target=onboardingTarget(),spot=document.getElementById("onboarding-spotlight");if(!target||!spot)return;const r=target.getBoundingClientRect(),pad=7;spot.style.left=`${Math.max(6,r.left-pad)}px`;spot.style.top=`${Math.max(6,r.top-pad)}px`;spot.style.width=`${Math.min(window.innerWidth-12,r.width+pad*2)}px`;spot.style.height=`${Math.min(window.innerHeight-12,r.height+pad*2)}px`;}
function renderOnboardingStep(){const item=ONBOARDING_STEPS[onboardingIndex],title=document.getElementById("onboarding-title"),copy=document.getElementById("onboarding-copy"),tip=document.getElementById("onboarding-tip"),progress=document.getElementById("onboarding-progress"),prev=document.getElementById("onboarding-prev"),next=document.getElementById("onboarding-next");if(!item)return;if(title)title.textContent=item.title;if(copy)copy.textContent=item.copy;if(tip)tip.textContent=item.tip;if(progress)progress.innerHTML=ONBOARDING_STEPS.map((step,index)=>`<span class="onboarding-dot ${index===onboardingIndex?"is-active":""}" aria-label="Langkah ${index+1} dari ${ONBOARDING_STEPS.length}"></span>`).join("");if(prev){prev.disabled=onboardingIndex===0;prev.setAttribute("aria-hidden",String(onboardingIndex===0));}if(next)next.textContent=onboardingIndex===ONBOARDING_STEPS.length-1?"Selesai":"Berikutnya";positionOnboardingSpotlight();}
function showOnboarding(){const tour=document.getElementById("onboarding-tour");if(!tour)return;clearTimeout(onboardingCloseTimer);onboardingIndex=0;tour.hidden=false;tour.classList.remove("is-closing");tour.classList.remove("is-open");tour.setAttribute("aria-hidden","false");document.body.classList.add("onboarding-open");requestAnimationFrame(()=>{tour.classList.add("is-open");renderOnboardingStep();document.getElementById("onboarding-next")?.focus();});}
function closeOnboarding(markSeen=true){const tour=document.getElementById("onboarding-tour");if(!tour||tour.hidden)return;if(markSeen)onboardingMarkSeen();tour.classList.remove("is-open");tour.classList.add("is-closing");const finish=()=>{tour.hidden=true;tour.classList.remove("is-closing");tour.setAttribute("aria-hidden","true");document.body.classList.remove("onboarding-open");const spot=document.getElementById("onboarding-spotlight");if(spot){spot.removeAttribute("style");}onboardingCloseTimer=null;};if(window.matchMedia?.("(prefers-reduced-motion: reduce)").matches)finish();else onboardingCloseTimer=setTimeout(finish,220);}
function maybeStartOnboarding(){if(!onboardingSeen()&&document.getElementById("onboarding-tour")?.hidden)setTimeout(()=>{if(!onboardingSeen()&&!document.body.classList.contains("public-mode"))showOnboarding();},420);}
...
function startNewFromEntry(){closeEntryGate(false);state=createNewProjectState();renderView();scrollWorkspaceTop();focusScreenHeading();showToast("Workspace baru siap diisi.");maybeStartOnboarding();}
function resumeFromEntry(){const saved=storedWorkspace();if(!saved){showToast("Belum ada workspace lokal untuk dilanjutkan.");return;}closeEntryGate(false);state=normalizeState(saved);renderView();maybeStartOnboarding();}
function emptyWorkspaceFromEntry(){closeEntryGate(false);state=createNewProjectState();state.view="home";state.projectName="Belum ada proyek";state.documents=[];renderView();maybeStartOnboarding();}
...
async function clearAllLocalData(){
  if(!window.confirm("Hapus seluruh state dan dokumen yang tersimpan di browser ini? File pada folder perangkat tidak akan dihapus."))return;
  try{localStorage.removeItem(STORAGE_KEY);localStorage.removeItem(LEGACY_STORAGE_KEY);localStorage.removeItem(ONBOARDING_KEY);}catch{}
  try{if(window.indexedDB){const request=indexedDB.deleteDatabase(DOC_DB_NAME);await new Promise(resolve=>{request.onsuccess=request.onerror=request.onblocked=resolve;});}}catch{}
  documentDirectoryHandle=null;documentDbPromise=null;state=createNewProjectState();state.view="home";state.projectName="Belum ada proyek";state.documents=[];history.replaceState(null,"","#beranda");renderView();setPublicMode(true);try{localStorage.removeItem(STORAGE_KEY);localStorage.removeItem(LEGACY_STORAGE_KEY);}catch{}showToast("Data aplikasi di browser ini sudah dihapus. File di folder perangkat tetap aman.");
}
...
function heading(n,title,desc,actions=""){return `<div class="heading"><div><p class="step-context">Langkah ${n} dari 5</p><h2 id="screen-heading" tabindex="-1">${title}</h2><p>${desc}</p></div>${actions?`<div class="heading-actions">${actions}</div>`:""}</div>`;}
function focusScreenHeading(){const target=document.getElementById("screen-heading");if(target)requestAnimationFrame(()=>target.focus({preventScroll:true}));}
function renderSteps(){document.getElementById("steps").innerHTML=labels.map((x,i)=>`<button type="button" class="step ${i===state.step?"active":""} ${i<state.step?"done":""}" data-step="${i}" aria-label="Langkah ${i+1}: ${x}" aria-current="${i===state.step?"step":"false"}"><span>${i<state.step?"S":i+1}</span><small>${x}</small></button>`).join("");}
...
function renderProfile(){
  ...
  <div class="label"><b>Proses yang relevan</b><small>Pilih yang benar-benar dilakukan</small></div><div class="activity-grid">${specific.activities.map(x=>`<button type="button" class="activity ${state.activities.includes(x)?"on":""}" data-activity="${esc(x)}" aria-pressed="${state.activities.includes(x)?"true":"false"}"><b>${esc(x)}</b><small>${state.activities.includes(x)?"Dipilih":"Klik untuk memilih"}</small></button>`).join("")}</div>
  ...
  <div class="label"><b>Pertanyaan penentu</b><small>Jawab sesuai kondisi proyek</small></div><div class="form-row dynamic-questions">${profile.questions.map(renderQuestion).join("")}</div>`;
}
...
function renderImpacts(){
  ...
  return heading(2,"Petakan sumber dampak yang relevan","Centang sumber yang benar-benar dihasilkan oleh proses atau fasilitas proyek. Kondisi tapak diisi pada langkah Lokasi.", `<button type="button" class="ghost" data-action="toggle-all-impacts">${state.showAllImpacts?"Tampilkan yang relevan":"Lihat semua sumber"}</button>`)+
  ...
  <div class="smart"><span>!</span><div><b>Pilih yang benar-benar ada</b><p>Centang hanya dampak yang berasal dari proses, fasilitas, bahan, atau kegiatan proyek.</p></div></div>
...
function renderComplianceRegister(){
  const r={...DEFAULT_COMPLIANCE_REVIEW,...(state.complianceReview||{})},last=r.changeLog?.at(-1),complete=officialResultIsComplete(r),pending=Boolean(r.officialDocumentType&&r.officialDocumentType!=="Belum ditentukan"&&r.officialDocumentType!=="Lainnya / perlu klarifikasi");
  const registerState=complete?`<span class="register-state confirmed">Hasil resmi lengkap untuk dicatat</span>`:pending?`<span class="register-state pending">Kandidat hasil · belum resmi</span>`:`<span class="register-state empty">Belum ada hasil resmi tercatat</span>`;
  return `<section class="compliance-register"><div class="validation-head"><div><small>REGISTER VERIFIKASI PROYEK</small><h3>Catat hasil resmi tanpa mengubahnya menjadi keputusan otomatis</h3><p>Isi hanya setelah ada hasil dari OSS/AMDALNet/instansi. Kolom ini tersimpan lokal dan dapat diekspor melalui state/ZIP.</p></div><span class="validation-mark">LOG</span></div>${registerState}${r.reScreenRequired?`<div class="review-alert"><b>Perlu re-screening</b><p>${esc(r.reScreenReason||"Ada perubahan data proyek.")} Periksa kembali KBLI, kapasitas, proses, lokasi, dampak, kewenangan, dan dokumen.</p><button type="button" class="soft compact" data-action="clear-rescreen">Tandai sudah ditinjau</button></div>`:""}<div class="compliance-register-grid"><label class="form-field"><span>Hasil penapisan resmi</span><select data-compliance-field="officialDocumentType">${["","Belum ditentukan","AMDAL","UKL-UPL","SPPL","Lainnya / perlu klarifikasi"].map(v=>`<option value="${esc(v)}" ${r.officialDocumentType===v?"selected":""}>${esc(v||"Pilih hasil resmi")}</option>`).join("")}</select></label><label class="form-field"><span>Status hasil</span><select data-compliance-field="officialDocumentStatus">${["Belum dikonfirmasi","Draf","Diajukan","Perlu perbaikan","Terbit / berlaku","Ditolak / dihentikan"].map(v=>`<option value="${esc(v)}" ${r.officialDocumentStatus===v?"selected":""}>${esc(v)}</option>`).join("")}</select></label><label class="form-field"><span>Nomor atau ID pengajuan</span><input data-compliance-field="officialDocumentNumber" value="${esc(r.officialDocumentNumber)}" placeholder="Wajib untuk hasil resmi"></label><label class="form-field"><span>Tanggal hasil</span><input type="date" data-compliance-field="officialDocumentDate" value="${esc(r.officialDocumentDate)}"></label><label class="form-field full"><span>Sumber/tautan hasil resmi</span><input data-compliance-field="officialDocumentSource" value="${esc(r.officialDocumentSource)}" placeholder="URL, nomor surat, atau nama kanal resmi"></label><label class="form-field"><span>Status kewenangan</span><select data-compliance-field="authorityStatus">${["Belum dikonfirmasi","Indikatif","Terkonfirmasi"].map(v=>`<option value="${esc(v)}" ${r.authorityStatus===v?"selected":""}>${esc(v)}</option>`).join("")}</select></label><label class="form-field"><span>Instansi/OPD yang dikonfirmasi</span><input data-compliance-field="authorityAgency" value="${esc(r.authorityAgency)}" placeholder="Wajib untuk hasil resmi"></label><label class="form-field"><span>Nomor tiket/surat kewenangan</span><input data-compliance-field="authorityReference" value="${esc(r.authorityReference)}" placeholder="Opsional bila tercantum pada sumber resmi"></label><label class="form-field"><span>Tanggal konfirmasi</span><input type="date" data-compliance-field="authorityDate" value="${esc(r.authorityDate)}"></label></div>${!complete&&pending?`<p class="register-requirements">Belum menggantikan kesimpulan acuan. Lengkapi: ${esc(officialResultMissing(r).join(", "))}.</p>`:""}${last?`<small class="change-note">Perubahan terakhir: ${esc(last.date)} · ${esc(last.reason)}</small>`:""}<p class="register-note">“Hasil resmi lengkap” hanya tampil jika jenis dan status akhir, nomor/ID atau sumber, tanggal hasil, instansi, konfirmasi kewenangan, serta tanggal kewenangan sudah diisi. Status ini tetap berupa catatan workspace; keputusan instansi tidak digantikan oleh aplikasi.</p></section>`;
}
...
function officialResultIsComplete(r){const type=String(r?.officialDocumentType||"");const status=String(r?.officialDocumentStatus||"");const finalStatus=["Terbit / berlaku","Ditolak / dihentikan"].includes(status),hasType=type&&type!=="Belum ditentukan"&&type!=="Lainnya / perlu klarifikasi",hasReference=Boolean(String(r?.officialDocumentNumber||"").trim()||String(r?.officialDocumentSource||"").trim());return Boolean(hasType&&finalStatus&&hasReference&&String(r?.officialDocumentDate||"").trim()&&r?.authorityStatus==="Terkonfirmasi"&&String(r?.authorityAgency||"").trim()&&(String(r?.authorityReference||"").trim()||String(r?.officialDocumentSource||"").trim())&&String(r?.authorityDate||"").trim());}
function officialResultMissing(r){const missing=[];if(!r?.officialDocumentType||r.officialDocumentType==="Belum ditentukan"||r.officialDocumentType==="Lainnya / perlu klarifikasi")missing.push("jenis dokumen");if(!["Terbit / berlaku","Ditolak / dihentikan"].includes(String(r?.officialDocumentStatus||"")))missing.push("status hasil akhir");if(!String(r?.officialDocumentNumber||"").trim()&&!String(r?.officialDocumentSource||"").trim())missing.push("nomor/ID atau sumber resmi");if(!String(r?.officialDocumentDate||"").trim())missing.push("tanggal hasil");if(r?.authorityStatus!=="Terkonfirmasi")missing.push("konfirmasi kewenangan");if(!String(r?.authorityAgency||"").trim())missing.push("instansi");if(!String(r?.authorityDate||"").trim())missing.push("tanggal kewenangan");return missing;}
function indicativeDocumentRecommendation(){
  ...
  const official=officialResultIsComplete(r),officialPending=Boolean(r.officialDocumentType&&r.officialDocumentType!=="Belum ditentukan"&&r.officialDocumentType!=="Lainnya / perlu klarifikasi");
  ...
  if(officialPending&&!official)explanation+=` Input jenis dokumen ${r.officialDocumentType} sudah dicatat, tetapi belum dianggap hasil resmi. Lengkapi ${officialResultMissing(r).join(", ")} pada register atau biarkan aplikasi memakai rekomendasi indikatif.`;
  ...
  const regulationBasis=primary.startsWith("AMDAL")?"PP 28/2025 untuk konteks perizinan berusaha, Permen LH/BPLH 22/2025 untuk kewenangan, Permen LHK 4/2021 untuk daftar kegiatan AMDAL/UKL-UPL/SPPL, serta PP 22/2021 dan aturan sektoral yang relevan.":primary.startsWith("UKL-UPL")?"PP 28/2025 dan Permen LH/BPLH 22/2025 untuk konteks perizinan/kewenangan, Permen LHK 4/2021 untuk daftar kegiatan, serta PP 22/2021 untuk pengelolaan dan pemantauan lingkungan.":"PP 28/2025 dan Permen LH/BPLH 22/2025 untuk konteks perizinan/kewenangan, Permen LHK 4/2021 dan hasil penapisan resmi sebagai acuan jalur SPPL, serta PP 22/2021 sebagai kerangka perlindungan lingkungan.";
...
const AMDAL_STUDY_PHASES=[
  {n:"01",name:"Penapisan",summary:"Tentukan rencana usaha, KBLI, skala, dan pendekatan studi.",anchor:"Data rencana usaha"},
  {n:"02",name:"Tim",summary:"Siapkan pemrakarsa dan penyusun sebelum data teknis diperiksa.",anchor:"Peran & kesiapan"},
  {n:"03",name:"Dokumen",summary:"Lengkapi tapak, peta, dan dokumen pendukung secara lokal.",anchor:"Tapak & dokumen"},
  {n:"04",name:"Pemeriksaan",summary:"Periksa kegiatan, parameter dampak, administrasi, dan kewenangan.",anchor:"Pelingkupan & review"},
  {n:"05",name:"Keputusan",summary:"Baca hasil indikatif, siapkan tindak lanjut, dan verifikasi resmi.",anchor:"Hasil & tindak lanjut"}
];
function renderAmdalStudy(){
  ...
  const polygonStatus=state.polygon?"Tersedia di browser":"Belum diunggah";
  const authorityStatus=state.province&&state.regency?`${state.regency}, ${state.province}`:"Belum dipilih";
  const pertekStatus=Object.values(state.answers||{}).some(value=>/ya|ada|digunakan dan disimpan/i.test(String(value)))?"Perlu ditinjau":"Belum ditinjau";
  ...
  return `<div class="view-shell amdal-study-shell">
    ${viewHeader("Studio AMDAL","Simulasi alur penapisan dan persetujuan lingkungan dengan data proyek EnviroTrack yang tersimpan lokal. Ini bukan AMDALNet resmi.",`<span class="amdal-mode-label">MODE LATIHAN · LOKAL</span>`)}
    <nav class="amdal-service-strip" aria-label="Layanan yang disimulasikan"><span class="active">Penapisan Otomatis</span><span>Asistensi Pelingkupan</span><span>AMDAL Digital Workspace</span></nav>
    <section class="amdal-atlas">
      <div class="amdal-atlas-copy"><p class="amdal-overline">ALUR PENAPISAN / ${phase.n}</p><h3>${esc(phase.name)}: ${esc(phase.summary)}</h3><p>Struktur tahap mengikuti istilah kerja yang terlihat pada AMDALNet publik: rencana usaha, tapak, kegiatan dan dampak, pemeriksaan, lalu hasil. Semua data dan hasil di sini adalah simulasi lokal; tidak ada pengajuan resmi.</p><div class="amdal-atlas-coordinates"><span>PROYEK</span><b>${esc(state.projectName||"Proyek baru")}</b><i></i><span>TAHAP</span><b>${esc(state.stage||"Pra-konstruksi")}</b></div></div>
      <div class="amdal-atlas-route" aria-label="Progres alur penapisan simulasi">${AMDAL_STUDY_PHASES.map((item,index)=>`<div class="route-node ${index===study.activePhase?"current":""} ${index<study.activePhase?"passed":""}"><span>${item.n}</span><b>${item.name}</b></div>`).join("")}</div>
    </section>
    <div class="amdal-study-layout">
      <section class="view-card amdal-phase-card"><div class="amdal-card-head"><div><p class="eyebrow">ALUR PENAPISAN</p><h3>Lima tahap yang dapat ditelusuri</h3></div><span>TAHAP ${String(study.activePhase+1).padStart(2,"0")}/05</span></div><div class="amdal-phase-list">${phaseRail}</div></section>
      <aside class="view-card amdal-project-card"><p class="eyebrow">KONTEKS PROYEK AKTIF</p><h3>${esc(state.projectName||"Proyek baru")}</h3><div class="amdal-project-facts">${projectRows}</div><div class="amdal-case-status"><span>STATUS WORKSPACE</span><b>Simulasi lokal</b></div><button type="button" class="soft amdal-open-screening" data-action="amdal-open-screening">Bandingkan di penapisan <span>→</span></button></aside>
    </div>
    <section class="amdal-workbench">
      <div class="view-card amdal-task-card"><div class="amdal-card-head"><div><p class="eyebrow">TAHAP ${phase.n}</p><h3>${esc(phase.name)}</h3></div><span class="amdal-registry">REG-${phase.n} · SIMULASI</span></div>
        ${study.activePhase===0?`<div class="amdal-lesson"><div class="amdal-question"><span>01</span><div><b>Mulai dari data rencana usaha.</b><p>Seperti alur publik AMDALNet, siapkan KBLI, skala, tahap kegiatan, dan pendekatan studi sebelum membaca hasil. EnviroTrack membantu menata data ini, bukan menerbitkan hasil resmi.</p></div></div><div class="amdal-process-list"><div><b>Data usaha</b><small>KBLI, nama kegiatan, kapasitas, dan tahap proyek.</small></div><div><b>Tapak</b><small>Lokasi, polygon, dan konteks kewenangan.</small></div><div><b>Kegiatan</b><small>Proses utama/pendukung dan parameter dampak.</small></div></div><div class="amdal-action-row"><button type="button" class="primary" data-action="amdal-open-screening">Buka data penapisan</button><span>Hasil latihan belum merupakan penetapan jenis dokumen.</span></div></div>`:study.activePhase===1?`<div class="amdal-lesson"><div class="amdal-question"><span>02</span><div><b>Pastikan peran dalam proses jelas.</b><p>AMDALNet membedakan pemrakarsa, penyusun, validator/pemeriksa, dan tim penilai. Studio hanya mensimulasikan kesiapan tim tanpa mengirim undangan atau menetapkan peran resmi.</p></div></div><div class="amdal-role-grid"><div><span>PEMRAKARSA</span><b>Pemberi data kegiatan</b></div><div><span>PENYUSUN</span><b>Penyusun dokumen</b></div><div><span>PEMERIKSA</span><b>Review administratif</b></div></div><div class="amdal-team-switch"><div><b>${study.teamReady?"Penugasan latihan telah diterima":"Menunggu tanggapan anggota"}</b><small>${study.teamReady?"Checkpoint tim dicatat hanya di browser ini.":"Simulasikan penerimaan penugasan untuk membuka checkpoint berikutnya."}</small></div><button type="button" class="${study.teamReady?"soft":"primary"}" data-action="amdal-team">${study.teamReady?"Tandai belum siap":"Terima penugasan latihan"}</button></div></div>`:study.activePhase===2?`<div class="amdal-lesson"><div class="amdal-question"><span>03</span><div><b>Lengkapi tapak dan dokumen pendukung.</b><p>Gunakan workspace Dokumen untuk mencatat bukti lokal, lalu cocokkan peta, status lahan, dan kesesuaian ruang secara manual. AMDALNet resmi dapat meminta format dan dokumen tertentu pada tahap pengajuan.</p></div></div><div class="amdal-document-grid"><div><span>PETA TAPAK</span><b>${esc(polygonStatus)}</b></div><div><span>DOKUMEN LOKAL</span><b>${(state.documents||[]).length} berkas tercatat</b></div><div><span>KEWENANGAN</span><b>${esc(authorityStatus)}</b></div></div></div>`:study.activePhase===3?`<div class="amdal-lesson"><div class="amdal-question"><span>04</span><div><b>Periksa pelingkupan, administrasi, dan kewenangan.</b><p>Bandingkan kegiatan utama/pendukung, parameter dampak, bukti, dan instansi yang berwenang. Status di bawah adalah penanda kerja lokal, bukan hasil pemeriksaan pejabat.</p></div></div><div class="amdal-review-grid"><div><span>KEGIATAN & DAMPAK</span><b>${state.impacts?.length?`${state.impacts.length} dipetakan`:"Belum dipetakan"}</b></div><div><span>STATUS DOKUMEN</span><b>${esc(reviewStatus)}</b></div><div><span>PERTEK</span><b>${esc(pertekStatus)}</b></div></div><div class="amdal-review-note"><span>RE-SCREEN</span><b>${state.complianceReview?.reScreenRequired?"Perlu ditinjau ulang":"Belum ada penanda peninjauan ulang"}</b><small>${esc(state.complianceReview?.reScreenReason||"Masukkan atau ubah data proyek untuk mempelajari pemicu peninjauan.")}</small></div></div>`:`<div class="amdal-lesson"><div class="amdal-question"><span>05</span><div><b>Baca hasil sebagai acuan, lalu lanjutkan verifikasi resmi.</b><p>Hasil lokal membantu menyiapkan pertanyaan, dokumen, dan bukti. Jenis dokumen final, kewenangan, dan keputusan persetujuan tetap mengikuti proses resmi AMDALNet/OSS dan instansi berwenang.</p></div></div><div class="amdal-result-box"><span>HASIL PENAPISAN LOKAL</span><b>${esc(documentHint)}</b><small>Status: ${esc(reviewStatus)} · Belum menjadi dokumen resmi</small></div><div class="amdal-action-row"><button type="button" class="soft" data-view="regulations" data-reg-open="all">Buka basis regulasi</button><a class="action-link" href="${LINKS.amdalnet}" target="_blank" rel="noopener noreferrer">Buka AMDALNet resmi ↗</a></div></div>`}
      </div>
      <aside class="view-card amdal-knowledge-card"><p class="eyebrow">CEK PEMAHAMAN</p><h3>Checkpoint tim</h3><p>Sebelum pekerjaan dokumen berjalan, apa fungsi tanggapan penugasan?</p><div class="amdal-answer-options"><button type="button" data-amdal-check="false">Merapikan tampilan dashboard</button><button type="button" data-amdal-check="true">Mencatat kesiapan peran dan tanggung jawab</button><button type="button" data-amdal-check="false">Menerbitkan hasil persetujuan</button></div>${study.knowledgeChecked?`<div class="amdal-feedback">✓ Tepat. Kesiapan peran adalah checkpoint proses, bukan persetujuan resmi.</div>`:""}</aside>
    </section>
    <p class="amdal-disclaimer">Studio AMDAL ini meniru struktur kerja publik yang dapat diamati pada AMDALNet, tetapi bukan salinan, bukan pengganti, dan tidak terhubung untuk mengirim data ke AMDALNet, OSS, atau instansi.</p>
  </div>`;
}
...
function localKbliResults(query){
  const q=String(query||"").trim().toLowerCase(),tokens=q.split(/\s+/).filter(Boolean),catalog=window.ENVIRO_KBLI_2025||{};
  return Object.entries(catalog).filter(([code,item])=>{const hay=`${code} ${item?.title||""} ${item?.description||""}`.toLowerCase();return tokens.every(token=>hay.includes(token));}).sort(([a,aa],[b,bb])=>{const ar=String(a).startsWith(q)?0:String(aa?.title||"").toLowerCase().startsWith(q)?1:2,br=String(b).startsWith(q)?0:String(bb?.title||"").toLowerCase().startsWith(q)?1:2;return ar-br||String(a).localeCompare(String(b));}).slice(0,9).map(([code,item])=>({id:`local-${code}`,code,title:item?.title||"",description:item?.description||""}));
}
function renderKbliSuggestions(results,source="local"){
  const list=document.getElementById("kbli-suggestions");if(!list)return;
  list.innerHTML=results.length?`<small class="kbli-source-note">${source==="local"?"Katalog lokal KBLI 2025":"Hasil pencarian OSS resmi · verifikasi sebelum dipakai"}</small>${results.map(r=>{const already=state.kblis.some(k=>k.code===String(r.code));return `<button type="button" class="suggestion ${already?"selected-result":""}" ${already?"disabled aria-disabled=\"true\"":`data-kbli-id="${esc(r.id)}" data-kbli-code="${esc(r.code)}" data-kbli-title="${esc(r.title)}" data-kbli-description="${esc(r.description)}"`}><strong>${esc(r.code)}</strong><div><b>${esc(r.title)}</b><small>${already?"Sudah dipilih":`${esc((r.description||"").slice(0,180))}${(r.description||"").length>180?"…":""}`}</small></div></button>`;}).join("")}`:`<div class="empty-note"><b>Tidak ada KBLI yang cocok di katalog lokal</b><p>Jika perlu, gunakan pencarian OSS resmi secara eksplisit.</p><button type="button" class="soft compact" data-action="remote-kbli-search" data-query="${esc(String(document.getElementById("kbli-search")?.value||""))}">Cari di OSS resmi ↗</button></div>`;
  list.classList.remove("hide");
}
async function searchKbliRemote(query){
  const list=document.getElementById("kbli-suggestions"),spinner=document.getElementById("kbli-spinner"),q=String(query||"").trim();if(!list||!spinner||q.length<2)return;
  spinner.classList.remove("hide");
  const url=`https://gw.oss.go.id/v2/portal/kbli?kategori=semua&search=${encodeURIComponent(q)}&lang=id&localization=id&limit=40&id_version=fff4053d-cbb0-51e9-9dc5-1e85b5740704`;
  try{const response=await fetch(url,{headers:{Accept:"application/json"}});if(!response.ok)throw new Error("OSS unavailable");const json=await response.json(),seen=new Set(),results=(json?.data?.result||[]).filter(r=>{const code=String(r?._source?.kode||"");if(!/^\d{5}$/.test(code)||seen.has(code))return false;seen.add(code);return true;}).slice(0,9).map(r=>{const s=r._source||{},loc=s.localization?.id||{};return{id:r._id,code:s.kode,title:loc.judul||"",description:loc.uraian||""};});renderKbliSuggestions(results,"remote");}catch{list.innerHTML=`<div class="empty-note"><b>Layanan KBLI OSS belum dapat dijangkau</b><p>Pencarian lokal tetap aman digunakan. Buka pencarian resmi OSS bila perlu.</p>${officialLink(LINKS.kbli,"Buka OSS KBLI")}</div>`;list.classList.remove("hide");}finally{spinner.classList.add("hide");}
}
async function searchKbli(query){
  const list=document.getElementById("kbli-suggestions"),spinner=document.getElementById("kbli-spinner"),q=String(query||"").trim();if(!list||!spinner)return;if(q.length<2){list.classList.add("hide");list.innerHTML="";return;}spinner.classList.remove("hide");renderKbliSuggestions(localKbliResults(q),"local");spinner.classList.add("hide");
}
...
document.addEventListener("keydown",e=>{const tour=document.getElementById("onboarding-tour");if(tour&&!tour.hidden){if(e.key==="Escape"){e.preventDefault();closeOnboarding();return;}if(e.key==="ArrowRight"||e.key==="Enter"){e.preventDefault();onboardingIndex=Math.min(ONBOARDING_STEPS.length-1,onboardingIndex+1);if(onboardingIndex===ONBOARDING_STEPS.length-1&&e.key==="Enter")closeOnboarding();else renderOnboardingStep();return;}if(e.key==="ArrowLeft"){e.preventDefault();onboardingIndex=Math.max(0,onboardingIndex-1);renderOnboardingStep();return;}if(e.key==="Tab"){const focusables=[...tour.querySelectorAll("button:not([disabled])")];if(focusables.length){const first=focusables[0],last=focusables[focusables.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}}}return;}if(e.key==="Escape"&&!document.getElementById("entry-gate")?.hidden)closeEntryGate();});
...
document.addEventListener("click",e=>{
  const onboardingAction=e.target.closest("[data-onboarding-action]");if(onboardingAction){const action=onboardingAction.dataset.onboardingAction;if(action==="skip"){closeOnboarding();return;}if(action==="prev"){onboardingIndex=Math.max(0,onboardingIndex-1);renderOnboardingStep();return;}if(action==="next"){if(onboardingIndex>=ONBOARDING_STEPS.length-1){closeOnboarding();return;}onboardingIndex+=1;renderOnboardingStep();return;}}
  if(!e.target.closest(".search-wrap")){document.getElementById("kbli-suggestions")?.classList.add("hide");}
  if(e.target.closest("#back")){state.step=Math.max(0,state.step-1);renderView();scrollWorkspaceTop();focusScreenHeading();return;}
  if(e.target.closest("#next")){
    ...
    if(state.step===4){state.view="projects";}else state.step=Math.min(4,state.step+1);renderView();scrollWorkspaceTop();if(state.view==="screening")focusScreenHeading();return;
  }
  ...
  const directAction=e.target.closest("[data-action]");if(directAction?.dataset.action==="open-onboarding"){e.preventDefault();showOnboarding();return;}if(directAction?.dataset.action==="new-project"){e.preventDefault();state=createNewProjectState();renderView();scrollWorkspaceTop();focusScreenHeading();showToast("Workspace baru siap diisi.");return;}
  const viewEl=e.target.closest("[data-view]");if(viewEl){e.preventDefault();if(viewEl.dataset.view==="screening"){state=createNewProjectState();if(viewEl.dataset.step!==undefined)state.step=+viewEl.dataset.step;}else{state.view=viewEl.dataset.view;if(state.view==="documents")state.documentTask=null;if(state.view==="regulations")state.regMode=viewEl.dataset.regOpen||"all";if(viewEl.dataset.step!==undefined)state.step=+viewEl.dataset.step;}renderView();if(state.view==="screening"){scrollWorkspaceTop();focusScreenHeading();}return;}
  ...
  const step=e.target.closest("[data-step]");if(step){state.view="screening";state.step=+step.dataset.step;renderView();scrollWorkspaceTop();focusScreenHeading();return;}
  ...
  if(a==="amdal-open-screening"){state.view="screening";state.step=2;renderView();scrollWorkspaceTop();focusScreenHeading();return;}
  ...
  if(a==="clear-local-data"){clearAllLocalData();return;}
  if(a==="remote-kbli-search"){searchKbliRemote(action.dataset.query||"");return;}
