...
function saveState(silent=true){
  try{localStorage.setItem(STORAGE_KEY,JSON.stringify({...state,_schemaVersion:2}));}catch{showToast("Draf tidak dapat disimpan di perangkat ini.");}
...
function renderDocuments(){
  ...
  const actions=`<input class="hide" id="doc-file-input" type="file" multiple><button type="button" class="primary" data-action="upload-doc">+ ${focusTask?"Tambah bukti tugas":"Tambah dokumen"}</button>`;
...
document.addEventListener("click",e=>{
  ...
  if(e.target.closest("#back")){state.step=Math.max(0,state.step-1);renderView();return;}
  if(e.target.closest("#next")){
    ...
    if(state.step===4){state.view="projects";}else state.step=Math.min(4,state.step+1);renderView();return;
  }
