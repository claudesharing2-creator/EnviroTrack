...
function buildTasks(){
  ...
  if(state.polygon||state.locationFlags.length||hasGroup("location"))tasks.push({id:"location-overlay",title:"Verifikasi overlay dan sensitivitas lokasi",cat:"Lokasi",status:state.polygon&&polygonCheck.valid?"ready":"blocked",due:"Hari 5",rule:"Tata ruang & kawasan",owner:"GIS",reason:"Hutan, konservasi, gambut, karst, pesisir, sempadan, dan kawasan bencana dapat mengubah jalur persetujuan. WMS yang tampil belum membuktikan irisan.",evidence:"Polygon, hasil overlay, tanggal dan sumber layer, query/feature evidence bila tersedia, serta konfirmasi pengelola kawasan.",system:[state.province==="Kalimantan Timur"?"WebGIS Kaltim":ptsp.name,state.province==="Kalimantan Timur"?LINKS.gisKaltim:ptsp.url],ruleUrl:LINKS.jdihn});
...
function renderProfile(){
  ...
  const selectedCard=state.kblis.length
    ? `<div class="kbli-list">${state.kblis.map((item,index)=>`<div class="selected-kbli ${index?"supporting":"primary-kbli"}"><span class="kbli-code">${esc(item.code)}</span><div class="kbli-copy"><span class="kbli-role">${index?"Pendukung":"Utama"}</span><b>${esc(item.title)}</b><small>${esc(item.description || "Uraian mengikuti KBLI 2025.")}</small></div><div class="kbli-actions">${index?`<button type="button" class="soft compact" data-action="make-primary" data-code="${esc(item.code)}">Jadikan utama</button>`:""}<button type="button" class="icon-button" data-action="remove-kbli" data-code="${esc(item.code)}" aria-label="Hapus KBLI ${esc(item.code)}">×</button></div></div>`).join("")}</div>`
...
function renderImpacts(){
  ...
  return heading(2,"Petakan sumber dampak yang relevan",`Daftar digabung dan disaring dari ${state.kblis.length||1} KBLI: ${profileNames}. Pilihan proses dan jawaban profil menentukan sumber yang muncul.`, `<button type="button" class="ghost" data-action="toggle-all-impacts">${state.showAllImpacts?"Tampilkan yang relevan":"Lihat semua sumber"}</button>`)+
...
function locationAssessmentMarkup(){const q=polygonQuality(state.polygon),ready=window.ENVIRO_OFFICIAL_MAP_LAYERS.filter(layer=>layer.endpoint&&layer.layerName),pending=window.ENVIRO_OFFICIAL_MAP_LAYERS.filter(layer=>!(layer.endpoint&&layer.layerName)),flags={estate:"Kawasan industri", "forest-cross":"Kawasan hutan", "marine-cross":"Ruang laut/pesisir", "cross-admin":"Lintas administrasi", protected:"Kawasan lindung/konservasi", "river-buffer":"Sempadan sungai/danau"},selectedFlags=(state.locationFlags||[]).map(key=>flags[key]).filter(Boolean),rows=[{label:"Kualitas polygon",status:!state.polygon?"Belum diunggah":q.valid?"Siap diperiksa":"Perlu diperbaiki",tone:!state.polygon?"todo":q.valid?"review":"todo",note:!state.polygon?"Unggah GeoJSON/KML untuk memulai pemeriksaan geometri.":`${q.points.length} titik unik · ${formatAreaM2(q.areaM2)}${q.warnings.length?` · ${q.warnings.join(" ")}`:""}${q.problems.length?` ${q.problems.join(" ")}`:""}`},{label:"Overlay confirmed",status:!state.polygon?"Menunggu polygon":"Belum dapat dinilai",tone:!state.polygon?"todo":"review",note:`${ready.length} service resmi tersedia. WMS visual belum menghitung irisan polygon; hasil spasial tetap perlu feature query atau konfirmasi instansi.`},{label:"Tema pending",status:`${pending.length} pending`,tone:"pending",note:"Layer tanpa endpoint resmi tidak dikirim request dan tidak memicu kewajiban otomatis."},{label:"Kewenangan administratif",status:state.province&&state.regency?"Indikatif":"Perlu dilengkapi",tone:state.province&&state.regency?"review":"todo",note:state.province&&state.regency?`${state.regency}, ${state.province}. Konfirmasi jalur PTSP/OPD pada sumber daerah sebelum pengajuan.`:"Pilih provinsi dan kabupaten/kota untuk membentuk konteks kewenangan."},{label:"Flag lokasi manual",status:selectedFlags.length?`${selectedFlags.length} dipilih`:"Belum ada",tone:selectedFlags.length?"review":"todo",note:selectedFlags.length?selectedFlags.join(", "):"Centang flag hanya jika sudah memiliki dasar dokumen atau hasil pemeriksaan lapangan."}];return `<section class="location-assessment" aria-labelledby="location-assessment-title"><div class="location-assessment-head"><div><small class="kicker">HASIL PENILAIAN TAPAK</small><h3 id="location-assessment-title">Status data dan pemicu kerja</h3><p>Panel ini membantu menentukan langkah verifikasi. Ia tidak menyimpulkan kesesuaian ruang atau kewajiban legal dari tampilan WMS.</p></div><span class="assessment-count">${rows.filter(row=>row.tone!=="review").length} perlu review</span></div><div class="location-assessment-list">${rows.map(row=>`<div class="location-assessment-row"><span class="assessment-dot ${row.tone}"></span><div><b>${esc(row.label)}</b><strong class="assessment-status ${row.tone}">${esc(row.status)}</strong><p>${esc(row.note)}</p></div></div>`).join("")}</div></section>`;}
...
function environmentalValidation(){
  const hasKbli=!!selectedKbli(),hasCapacity=!!state.capacity&&!!state.capacityUnit,hasLocation=!!state.province&&!!state.regency,hasImpacts=state.impacts.length>0,hasFlora=state.impacts.includes("flora"),hasFauna=state.impacts.includes("fauna"),hasB3=hasGroup("b3"),hasB3Candidates=state.wasteCodes.length>0,hasPollution=hasGroup("wastewater")||hasGroup("emission")||hasB3;
  return [
    {id:"screening",label:"Jalur AMDAL / UKL-UPL / SPPL",tone:hasKbli&&hasCapacity&&hasLocation?"review":"todo",status:hasKbli&&hasCapacity&&hasLocation?"Siap ditelaah":"Perlu dilengkapi",note:"Cocokkan KBLI, skala, teknologi, lokasi, dan kegiatan pendukung dengan daftar resmi; hasil aplikasi tetap indikatif.",source:"plhk4-2021"},
    {id:"impact",label:"Dampak dan rona awal",tone:hasImpacts&&hasFlora&&hasFauna?"review":"todo",status:hasImpacts&&hasFlora&&hasFauna?"Perlu konfirmasi":"Perlu dilengkapi",note:hasFlora&&hasFauna?"Flora dan fauna sudah dipisahkan; lengkapi sumber, penerima, rona awal, metode, indikator, dan rencana pemantauan.":"Pilih dampak yang relevan dan periksa flora serta fauna secara terpisah.",source:"pp22-2021"},
    {id:"b3",label:"Jenis dan kode limbah B3",tone:!hasB3?"na":hasB3Candidates?"review":"todo",status:!hasB3?"Tidak terpicu":hasB3Candidates?"Perlu telusur sumber":"Perlu dipetakan",note:!hasB3?"Belum ada sumber B3 yang dipilih.":"Setiap kandidat harus dicocokkan dengan sumber proses, bahan, karakteristik, jumlah, kemasan, dan kondisi aktual.",source:"plhk6-2021"},
    {id:"technical",label:"Persetujuan Teknis / SLO",tone:hasPollution?"review":"na",status:hasPollution?"Perlu diperiksa":"Belum terpicu",note:hasPollution?"Periksa kebutuhan Persetujuan Teknis atau SLO berdasarkan jenis pencemar, jalur pengelolaan, dan ketentuan yang berlaku.":"Akan muncul bila pilihan memicu air limbah, emisi, atau limbah B3.",source:"plhk5-2021"},
    {id:"authority",label:"Kewenangan dan jalur OSS",tone:"review",status:"Perlu konfirmasi",note:"Gunakan konteks perizinan berusaha terbaru dan konfirmasi kewenangan penerbit pada OSS/instansi terkait.",source:"pp28-2025"}
  ];
}
function renderValidationPanel(compact=false){const rows=environmentalValidation();return `<section class="validation-panel ${compact?"compact":""}"><div class="validation-head"><div><small>VALIDASI REGULASI & DATA</small><h3>${compact?"Catatan sebelum mengandalkan hasil":"Pemeriksaan awal AMDAL / UKL-UPL"}</h3><p>Ini adalah pemeriksaan kelengkapan dan jejak sumber, bukan keputusan kelayakan lingkungan atau pengganti pemeriksaan instansi.</p></div><span class="validation-mark">CHECK</span></div><div class="validation-list">${rows.map(row=>{const rule=REGULATIONS.find(r=>r.id===row.source);return `<div class="validation-row"><span class="validation-dot ${row.tone}"></span><div><b>${esc(row.label)}</b><strong class="validation-status ${row.tone}">${esc(row.status)}</strong><p>${esc(row.note)}</p>${rule?officialLink(rule.url,`Sumber: ${rule.title}`,"inline-link"):""}</div></div>`}).join("")}</div><div class="validation-foot">Rujukan kerja diperiksa ${esc("23 Agu 2026")}; cek kembali versi resmi dan aturan sektoral/daerah sebelum pengajuan.</div></section>`;}
...
function progressForTask(id){const value=state.taskProgress?.[taskProgressKey(id)]||(state.projectName===DEFAULT_STATE.projectName?state.taskProgress?.[id]:null);return value&&typeof value==="object"?{status:value.status||"not_started",done:Array.isArray(value.done)?value.done:[],markedAt:value.markedAt||""}:{status:"not_started",done:[],markedAt:""};}
...
function taskStatusActions(task,progress){
  ...
  if(progress.status==="self_declared")return `<button type="button" disabled>Sudah dilakukan</button><button type="button" class="secondary" data-action="undo-existing" data-id="${task.id}">Batalkan tanda</button>`;
  if(progress.status==="completed")return `<button type="button" disabled>Selesai terverifikasi</button><button type="button" class="secondary" data-action="reopen-task" data-id="${task.id}">Buka kembali</button>`;
...
function renderAllTasks(){
  ...
  else if(state.taskFilter==="completed")shown=all.filter(x=>["self_declared","evidence_needed","completed"].includes(progressForTask(x.id).status));
...
function calendarTaskStatus(task){const status=progressForTask(task.id).status;return status==="completed"?"Selesai":status==="evidence_needed"?"Butuh bukti":status==="in_progress"?"Berjalan":status==="self_declared"?"Dinyatakan selesai":"Belum dimulai";}
...
function setKbli(data){
  ...
  state.activities=profile.activities.slice(0,3);state.capacity="";state.capacityUnit=profile.units[0];state.answers={};state.impacts=[...profile.defaults];state.wasteCodes=profile===QUESTIONNAIRE_PROFILES.migas?["b105d","b110d"]:[];state.showAllImpacts=false;state.showAllWaste=false;
...
function makePrimaryKbli(code){
  ...
  const profile=profileForCode(code);
  state.activities=profile.activities.slice(0,3);state.capacity="";state.capacityUnit=profile.units[0];state.answers={};state.showAllImpacts=false;state.showAllWaste=false;addTriggeredImpacts(profile.defaults);state.projectName=item.title;
...
document.addEventListener("change",e=>{
  ...
  if(e.target.matches("[data-task-step]")){
    ...
    const done=e.target.checked?[...new Set([...progress.done,index])]:progress.done.filter(x=>x!==index),total=guideForTask(task).steps.length;
    const hasEvidence=documentsForTask(id).length>0,status=done.length===total?(hasEvidence?"completed":"evidence_needed"):"in_progress";
...
document.addEventListener("submit",e=>{
  if(e.target.id!=="calendar-event-form")return;e.preventDefault();const data=new FormData(e.target),title=String(data.get("title")||"").trim(),date=String(data.get("date")||""),owner=String(data.get("owner")||"").trim();if(!title||!date||!owner){showToast("Lengkapi nama agenda, tanggal, dan penanggung jawab.");return;}state.calendarEvents=[...(state.calendarEvents||[]),{id:`event-${Date.now()}`,title,date,owner,taskId:String(data.get("taskId")||"")||null,status:"Dijadwalkan"}];saveState();renderView();showToast("Agenda ditambahkan dan disimpan di workspace lokal.");
});
document.addEventListener("click",e=>{
  ...
  if(a==="remove-polygon"){state.polygon=null;saveState();renderView();showToast("Polygon dihapus dari workspace lokal.");return;}
  ...
  if(a==="mark-existing"){
    ...
    const done=guideForTask(task).steps.map((_,i)=>i),markedAt=new Intl.DateTimeFormat("id-ID",{day:"2-digit",month:"short",year:"numeric"}).format(new Date()),status=documentsForTask(id).length?"completed":"self_declared";
    state.taskProgress[taskProgressKey(id)]={status,done,markedAt};state.openTask=id;saveState();renderView();showToast(status==="completed"?"Tugas ditandai selesai dan bukti yang sudah ada ditemukan.":"Tugas ditandai sudah dilakukan. Bukti dapat ditambahkan sekarang atau nanti.");
  }
