...
function renderLocation(){
  ...
  `<input class="hide" id="polygon-file-input" type="file" accept=".geojson,.json,.kml,application/geo+json,application/json,text/xml"><div class="map ${polygon?"has-polygon":""}"><span class="island one"></span><span class="island two"></span><span class="island three"></span><span class="pin" aria-hidden="true"></span><div class="mapbox"><b>${polygonLabel}</b><small>${polygonNote}</small><div class="map-actions"><button type="button" class="primary" data-action="upload-polygon">${polygon?"Ganti polygon":"Unggah polygon"}</button>${polygon?'<button type="button" class="soft" data-action="remove-polygon">Hapus</button>':""}</div></div></div>
...
function renderCalendar(){
  ...
  const completed=entries.filter(item=>["Selesai","Terverifikasi"].includes(item.status)).length,linkedDocs=(state.documents||[]).filter(doc=>(doc.projectName||state.projectName)===state.projectName&&(doc.taskIds||[]).length).length;
...
function renderView(){
  ...
  content.classList.remove("page-enter","page-enter-active");content.classList.add("page-enter");requestAnimationFrame(()=>{content.classList.add("page-enter-active");initMotion();});
