...
const TASK_GUIDES={
  entity:{portalWhen:"Buka AHU setelah bentuk usaha, nama, pendiri/pengurus, modal, alamat, maksud-tujuan, dan pemilik manfaat sudah disepakati. Untuk PT biasa, proses portal dilakukan bersama notaris.",steps:["Pilih bentuk usaha: PT biasa, Perseroan Perorangan, CV, koperasi, atau bentuk lain.","Tetapkan nama, alamat kedudukan, pendiri/pemegang saham, pengurus, modal, dan pemilik manfaat.","Cocokkan maksud-tujuan dalam dokumen pendirian dengan KBLI yang benar-benar akan dijalankan.","Minta notaris menyiapkan dan mengulas draf akta—atau gunakan jalur mandiri jika bentuk usaha memperbolehkannya.","Pastikan SK pengesahan/bukti pendaftaran terbit dan simpan dokumen final sebagai bukti tugas."]},
...
function taskRows(tasks){
  return `<div class="tasklist">${tasks.map(x=>{
    const guide=guideForTask(x),progress=progressForTask(x.id),done=Array.isArray(progress.done)?progress.done:[],docs=documentsForTask(x.id),percent=Math.round(done.length/guide.steps.length*100);
    const runtime=progress.status==="completed"?"completed":progress.status==="evidence_needed"?"evidence-needed":progress.status==="in_progress"?"in-progress":x.status;
    const runtimeLabel=progress.status==="completed"?"Selesai":progress.status==="evidence_needed"?"Langkah selesai · bukti belum ada":progress.status==="in_progress"?"Sedang dikerjakan":x.status==="later"?"Nanti / praoperasi":x.status==="blocked"?"Menunggu prasyarat":"Belum dimulai";
    return `<article class="task"><button class="task-main" data-task="${x.id}"><span class="task-state ${runtime}">${runtime==="completed"?"✓":runtime==="evidence-needed"?"!":runtime==="in-progress"?"◐":x.status==="later"?"◷":x.status==="blocked"?"↳":""}</span><span class="task-copy"><small>${esc(x.cat)}</small><b>${esc(x.title)}</b><em>${esc(x.rule)} · ${runtimeLabel}</em></span><span class="owner">${esc(x.owner)}</span><span class="due">${esc(x.due)}</span><span>${state.openTask===x.id?"⌃":"⌄"}</span></button>${state.openTask===x.id?`<div class="task-detail"><div class="task-summary-grid"><div><small>MENGAPA WAJIB</small><p>${esc(x.reason)}</p></div><div><small>HASIL / BUKTI YANG DIBUTUHKAN</small><p>${esc(x.evidence)}</p></div></div><section class="task-guide"><div class="guide-head"><div><small>CARA MENGERJAKAN</small><b>${progress.status==="not_started"?"Baca dulu, lalu mulai tugas":progress.status==="evidence_needed"?"Semua langkah selesai—tambahkan bukti agar tugas selesai":`${done.length} dari ${guide.steps.length} langkah selesai`}</b></div><span>${percent}%</span></div><div class="guide-progress"><span style="width:${percent}%"></span></div><ol>${guide.steps.map((step,index)=>`<li class="${done.includes(index)?"done":""}"><label><input type="checkbox" data-task-step="${x.id}" data-step-index="${index}" ${done.includes(index)?"checked":""} ${progress.status==="not_started"?"disabled":""}><span class="guide-number">${index+1}</span><span>${esc(step)}</span></label></li>`).join("")}</ol><div class="portal-when"><span>i</span><p><b>Kapan membuka ${esc(x.system[0])}?</b>${esc(guide.portalWhen)}</p></div></section><section class="task-evidence ${progress.status==="evidence_needed"?"needs-evidence":""}"><div><small>DOKUMEN TERKAIT TUGAS INI</small><b>${docs.length} dokumen sudah ditautkan</b><p>${docs.length?docs.map(d=>esc(d.name)).join(" · "):"Belum ada dokumen. Gunakan tombol Kelola bukti agar file masuk langsung ke tugas ini."}</p></div><button class="soft" data-action="manage-evidence" data-id="${x.id}">${progress.status==="evidence_needed"?"Tambahkan bukti":"Kelola bukti"} →</button></section><div class="task-links"><button data-action="start-task" data-id="${x.id}" ${progress.status!=="not_started"?"disabled":""}>${progress.status==="completed"?"Tugas selesai":progress.status==="evidence_needed"?"Menunggu bukti":progress.status==="in_progress"?"Sedang dikerjakan":"Mulai tugas"}</button><a class="secondary" href="${x.system[1]}" target="_blank" rel="noopener noreferrer">Buka ${esc(x.system[0])} ↗</a><a class="secondary" href="${x.ruleUrl}" target="_blank" rel="noopener noreferrer">Buka aturan ↗</a></div></div>`:""}</article>`;
  }).join("")}</div>`;
}
function renderTracker(){
  const all=buildTasks(),blocked=all.filter(x=>x.status==="blocked").length,later=all.filter(x=>x.status==="later").length,totalSteps=all.reduce((n,x)=>n+guideForTask(x).steps.length,0),doneSteps=all.reduce((n,x)=>n+(progressForTask(x.id).done||[]).length,0),percent=totalSteps?Math.round(doneSteps/totalSteps*100):0,active=all.filter(x=>["in_progress","evidence_needed"].includes(progressForTask(x.id).status)).length,complete=all.filter(x=>progressForTask(x.id).status==="completed").length;
  return `<div class="tracker-head"><div><span class="kicker">CHECKLIST PROYEK</span><h2>${esc(state.projectName)} — ${all.length} tugas</h2></div><button class="ghost" data-view="tasks">Lihat semua tugas</button></div><div class="progress"><div class="ring" style="background:conic-gradient(#287158 ${percent}%,#cbdcd4 0)"><b>${percent}%</b></div><div class="progress-info"><b>${active} sedang dikerjakan · ${complete} selesai</b><p>${doneSteps}/${totalSteps} langkah · ${blocked} menunggu prasyarat · ${later} praoperasi/operasi</p><div class="bar"><span style="width:${percent}%"></span></div></div></div>${taskWorkflowStrip()}${taskRows(all)}`;
}
...
document.addEventListener("change",e=>{
  if(e.target.matches("[data-task-step]")){
    ...
    state.taskProgress[id]={status,done};saveState();renderView();if(done.length===total)showToast(hasEvidence?"Semua langkah dan bukti tugas sudah lengkap.":"Semua langkah selesai. Tambahkan bukti agar tugas selesai.");return;
  }
